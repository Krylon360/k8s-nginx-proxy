...
K8S_SERVICE_NAME=$1
sed -i "s#{{K8S_SERVICE_NAME}}#${K8S_SERVICE_NAME}#g" ./nginx-template.env > nginx.env

K8S_DNS_HOST=$2
sed -i "s#{{K8S_DNS_HOST}}#${K8S_DNS_HOST}#g" ./nginx.env >> nginx.env

NGINX_PORT=$3
sed -i "s#{{NGINX_PORT}}#${NGINX_PORT}#g" ./nginx.env >> nginx.env
